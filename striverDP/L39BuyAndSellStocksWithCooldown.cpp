...
// using memoization
int f(int day, int canBuy ,vector<int> &prices,vector<vector<int>> &DP){
    if(day >= prices.size()) {
    ...
    if(canBuy == 1){  // means that this we can buy that stock
        ...
        int notBuy = 0 + f(day+1 , 1 ,prices,DP);

        profit = max(buy , notBuy);
    }else{   // bought stock can sell
        int sold = prices[day] + f(day+2 , 1 ,prices,DP);
        int notSold = 0 + f(day+1, 0 , prices,DP);
    ...
    for(int day = n-1 ; day>=0 ; day--){
        for(int canBuy = 0 ; canBuy<2 ; canBuy++){
            if(canBuy == 1){
                ...
                int sold = +prices[day] + DP[day+2][1];
